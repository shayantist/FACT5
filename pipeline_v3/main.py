...
from pipeline_v3.utils import chunk_text, print_header, retry_function
...
class SearchProvider:
    def __init__(self, provider: Literal["serper", "duckduckgo", "tavily"], api_key: str = None):
    ...
    def _tavily_search(self, query: str, num_results: int) -> List[SearchResult]:
    ...
    def _is_blacklisted_source(self, url: str) -> bool:
        ...
        domain = urlparse(url).netloc.lower()
        return any(rd in domain for rd in self.blacklisted_domains)
...
class ClaimExtractor(dspy.Module):
    ...
    def forward(self, statement: str) -> List[Claim]:
        ...
        try:
            ...
            # Error handling: if claims is still a dict, get the list of claims
...
class OverallStatementEvaluatorSignature(dspy.Signature):
    ...
    overall_verdict: VERDICTS = dspy.OutputField(desc="Single truthfulness verdict for the overall statement (do not treat each claim as of equal importance, weigh each claim depending on how much the statement depends on it)")
    confidence: float = dspy.OutputField(desc="Confidence score for the overall truthfulness verdict")
...
class FactCheckPipeline:
    ...
    def fact_check(self, statement: str, web_search: bool = True):
        ...
        for claim_i, claim in enumerate(claims, 1):
            # Keep track of queries seen to avoid queries+processing
            seen_queries = set()
                    ...
                    while not answered and retries_per_answer < self.max_retries:
                        ...
                        for query_i, query in enumerate(component.search_queries, 1): 
                            if query in seen_queries:
                                print_header(f"Skipping duplicate query: {colored(query, 'yellow')}", level=4)
                                continue
                            ...
                            if web_search:
                                ...
                                if VERBOSE:
                                    ...
                                    for i, result in enumerate(search_results, 1):
                                        ...
                                        print_header(f"Excerpt: {colored(f'{result.excerpt[:150]}...[{len(result.excerpt)} chars]', 'magenta')}", level=5)
                            ...
                            relevant_docs.extend(self.vector_store.retrieve(query, k=self.num_retrieved_docs))
                        ...
                        if VERBOSE: 
                            ...
                            for i, doc in enumerate(relevant_docs, 1):
                                ...
                                print_header(f"Snippet: {colored(f'{doc.content[:150]}...[{len(doc.content)} chars]', 'magenta')}", level=5)
                    ...
                    for j, component in enumerate(claim.components, 1):
                        ...
                        if component.answer.citations:
                            ...
                            for k, doc in enumerate(component.answer.retrieved_docs, 1):
                                print_header(f"[{k}] {colored(f'{doc.content[:150]}...[{len(doc.content)} chars]', 'magenta')}", level=5)
            ...
            for i, claim in enumerate(claims, 1):
                ...
                for j, component in enumerate(claim.components, 1):
                    ...
                    if component.answer.citations:
                        ...
                        print_header("No explicit citations made by LM, but relevant documents used to synthesize answer: ")
                        for k, doc in enumerate(component.answer.retrieved_docs, 1):
                            print_header(f"[{k}] {colored(f'{doc.content[:150]}...[{len(doc.content)} chars]', 'magenta')}", level=4)
...
if __name__ == "__main__":
    ...
    # Initialize DSPy
    lm = dspy.LM('gemini/gemini-2.0-flash', api_key=os.getenv('GOOGLE_GEMINI_API_KEY'), cache=False, max_tokens=8192)
    ...
    pipeline = FactCheckPipeline(
        model=lm,
        vector_store=VectorStore(model_name=EMBEDDING_MODEL, use_bm25=USE_BM25, bm25_weight=BM25_WEIGHT),
        # search_provider=SearchProvider(provider="serper", api_key=os.getenv('SERPER_API_KEY')),
        search_provider=SearchProvider(provider="tavily", api_key=os.getenv('TAVILY_API_KEY')),
        ...
        num_retries_per_answer=1,
        num_retries_per_claim=1,
    )
